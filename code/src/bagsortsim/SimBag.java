...
public class SimBag {
	...
	final static float cspeedmax = (63.0f/2.5f) / BagSortSim.freq;

	final static float dspeedmin = (56.0f/2.3f) / BagSortSim.freq;

	final static float dspeedmax = (56.0f/2.3f) / BagSortSim.freq;

	final static float espeedmin = (28.0f/1.1f) / BagSortSim.freq;

	final static float espeedmax = (28.0f/1.2f) / BagSortSim.freq;
	...
	//   On Distribution Belt.  mm from feed belt in "long" direction.
	...
	public SimBag(int chkin, int col) {
		...
		colorVal = (color==BagSortSim.YELLOW && sensor.isActivated()? 55 : 40);
	...
	int sensorValue(Float pos) {
        /*
		 * Simulate passing of sensor
		 * If sensor not activated, a yellow bag looks black!
		 */
