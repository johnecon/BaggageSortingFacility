...
public class BagSortSim {
	...
	public Vector<SimBag> bags = new Vector<SimBag>();
	...
	boolean feedCheck = true; // Should we check for empty feed belt?
	int separation = 10;      // Separation between bags [mm]
...
//			message("Checkin " + chkin + " not clear (bag ignored)");
	...
	public synchronized void setFeedCheck(boolean active) {
		if (feedCheck!= active) {
			feedCheck = active;
			window.newparams();
	...
	public synchronized void setSeparation(int mm) {
		if (mm < 1 || mm > 50) {
			warning("Separation out of range [1..50]");
			return;
		}
		if (separation != mm) {
			separation = mm;
...
class ParamPanel extends Panel {
	...
	Checkbox checkon = new Checkbox(" Max 1 on feed belt",true);
	...
	public ParamPanel(BagSortSim _sim) {
		...
		checkon.addItemListener(new ItemListener() {
	...
	public void newparams() {
		...
		checkon.setState(sim.getFeedCheck());  
		checkon.repaint();
